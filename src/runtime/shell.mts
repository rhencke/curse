...
/** Per-module `$LINENO` maps: a generated-module URL → its sorted
 *  [jsLine, shLine] pairs (sparse, at command boundaries). Populated at module
 *  load by the emitted `sh.mapLines(import.meta.url, …)` call and consulted
 *  lazily — only when `$LINENO` is actually read. */
const lineMaps = new Map<string, ReadonlyArray<readonly [number, number]>>();

/** Resolve `$LINENO` from the current call stack: find the innermost frame that
 *  belongs to a mapped module and translate its JS line to the source line.
 *  Returns undefined when no mapped frame is on the stack (e.g. the interpreter,
 *  which uses `sh.line` instead). */
const linenoFromStack = (): number | undefined => {
  if (lineMaps.size === 0) return undefined;
  const stack = new Error().stack;
  if (stack === undefined) return undefined;
  for (const frame of stack.split("\n")) {
    const m = /(?:\(| at )(\S+?):(\d+):\d+\)?\s*$/.exec(frame);
    if (m === null) continue;
    const pairs = lineMaps.get(m[1]!);
    if (pairs === undefined) continue;
    const js = Number(m[2]);
    // Largest jsLine <= the frame's line (the command that line belongs to).
    let sh = pairs.length > 0 ? pairs[0]![1] : 0;
    for (const [j, s] of pairs) { if (j > js) break; sh = s; }
    return sh;
  }
  return undefined;
};
...
export class Shell {
  ...
  private dynamicSpecial(name: string): string | undefined {
    ...
    // AOT resolves $LINENO lazily from the call stack via the module's line map
