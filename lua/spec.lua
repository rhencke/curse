...
local function json_decode(s)
  ...
  while i <= n do
    ...
    if c == "\\" then
      ...
      if e == "n" then o[#o + 1] = "\n"; i = i + 2
      elseif e == "t" then o[#o + 1] = "\t"; i = i + 2
      elseif e == "r" then o[#o + 1] = "\r"; i = i + 2
      elseif e == "b" then o[#o + 1] = "\b"; i = i + 2
      elseif e == "f" then o[#o + 1] = "\f"; i = i + 2
      elseif e == "0" then o[#o + 1] = "\0"; i = i + 2
      elseif e == "\\" or e == '"' or e == "/" then o[#o + 1] = e; i = i + 2
      elseif e == "x" then o[#o + 1] = string.char(tonumber(s:sub(i + 2, i + 3), 16) or 0); i = i + 4
      elseif e == "u" then
        if s:sub(i + 2, i + 2) == "{" then
          local hex = s:match("^{(%x+)}", i + 2)
          o[#o + 1] = utf8_encode(tonumber(hex, 16) or 0); i = i + 2 + #hex + 2
        else
          o[#o + 1] = utf8_encode(tonumber(s:sub(i + 2, i + 5), 16) or 0); i = i + 6
        end
      else o[#o + 1] = e; i = i + 2 end
...
local function shells_match_bash(shells)
  for sh in (shells .. "/"):gmatch("([%w]+)/") do if sh == "bash" or sh == "osh" then return true end end
  return false
end
...
local function parse_cases(text)
  ...
  for line in (text .. "\n"):gmatch("(.-)\n") do
    ...
    if name then
      ...
      cur = { name = name, code = {}, meta = {} }; inMeta = false
    elseif cur then
      if line == "##" or line:sub(1, 3) == "## " then inMeta = true; cur.meta[#cur.meta + 1] = line
      elseif inMeta then cur.meta[#cur.meta + 1] = line -- block body (e.g. STDOUT lines up to ## END)
      else cur.code[#cur.code + 1] = line end
    end
  end
  if cur then cases[#cases + 1] = cur end
  local out = {}
...
local function parse_meta(meta)
  ...
  while i <= m do
    ...
    if rest then
      ...
      if not kind then kind, shells, tail = rest:match("^(BUG)%s+([%w/]+)%s+(.*)$") end
      if not kind then kind, shells, tail = rest:match("^(N%-I)%s+([%w/]+)%s+(.*)$") end
      ...
      if st then if kind == nil then status.def = tonumber(st) elseif bashvar then status.bash = tonumber(st) end
      -- single-line stdout
      elseif body:match("^stdout:") then
        local v = body:match("^stdout:%s?(.*)$") or ""
        v = v .. "\n"
        if kind == nil then stdout.def = v elseif bashvar then stdout.bash = v end
      ...
      elseif body:match("^STDOUT:") then
        ...
        local lines = {}
        i = i + 1
        while i <= m and not meta[i]:match("^##%s+END") and not meta[i]:match("^##%s*$") and meta[i] ~= "## END" do
          -- stop if another directive starts (defensive; blocks normally end at ## END)
          if meta[i]:match("^##%s+%u") or meta[i]:match("^##%s+OK") or meta[i]:match("^##%s+BUG") or meta[i]:match("^##%s+N%-I") then break end
          lines[#lines + 1] = meta[i]; i = i + 1
        end
...
local function run(cmdstr, cwd, shval, pathpre)
  os.execute("cd " .. cwd .. " && { export LC_ALL=" .. LOCALE .. " TMP=" .. cwd .. " TMPDIR=" .. cwd ..
...
local function is_oil(path)
  ...
  return b:match("^ysh%-") or b:match("^hay") or b:match("^tea")
    or b:match("^osh%-") or b:match("%-osh%.test") -- OSH-specific files
end
if #filters > 0 then
  local kept = {}
...
for _, path in ipairs(files) do
  ...
  goldPass = goldPass + gp; livePass = livePass + lp; structPass = structPass + sp
