...
local function topic_matches(pat)
  for name in pairs(BUILTINS) do if name == pat or name:sub(1, #pat) == pat then return true end end
  for name in pairs(KEYWORDS) do if name == pat or name:sub(1, #pat) == pat then return true end end
  return false
end
...
return function(sh, cmd, args)
  ...
  while args[j] and args[j]:sub(1, 1) == "-" and #args[j] > 1 do
    if args[j] == "--" then j = j + 1; break end
    j = j + 1
  end
  if not args[j] then sh.status = 0; return end -- bare `help`: overview, always succeeds
  local allok = true
  for k = j, #args do
    local pat = args[k]
    if topic_matches(pat) then
      ...
      allok = false
      io.stderr:write("curse: help: no help topics match `" .. pat ..
        "'.  Try `help help' or `man -k " .. pat .. "' or `info " .. pat .. "'.\n")
    end
  end
