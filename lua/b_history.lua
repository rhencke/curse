	...
	if cmd == "history" then
		...
		elseif a == "-r" or a == "-n" then -- read history from FILE (default $HISTFILE)
			...
			if file and file ~= "" then
				...
				if f then
					...
					for k = from, #lines do
						sh.history[#sh.history + 1] = lines[k]
					end
		...
		elseif a == "-d" then -- delete the history entry at OFFSET (negative counts from end)
			...
			if idx and idx >= 1 and idx <= n then
				table.remove(sh.history, idx)
				sh.status = 0
			else
				io.stderr:write("curse: history: " .. tostring(args[3]) .. ": history position out of range\n")
				sh.status = 1
			end
		elseif a == "-w" or a == "-a" then -- write history to FILE
			...
			if f then
				for _, h in ipairs(sh.history) do
					f:write(h, "\n")
				end
				f:close()
			end
		...
		elseif a == nil then -- list the whole history
			for i = 1, #sh.history do
				sh:echo(("%5d  %s"):format(i, sh.history[i]))
			end
			sh.status = 0
		...
		elseif not tonumber((a:gsub("^%+", ""))) then -- a non-numeric count (`history f`)
			...
			for i = math.max(1, #sh.history - nn + 1), #sh.history do
				sh:echo(("%5d  %s"):format(i, sh.history[i]))
			end
			sh.status = 0
		end
