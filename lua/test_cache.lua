...
local function run(src)
  ...
  local b = {}; sh.out = function(s) b[#b + 1] = s end
...
local function ck(desc, got, how, want, wanthow)
  ...
  ok = ok and pass
  print(("  %-26s %-14s [%-7s] %s"):format(desc, got, how,
    pass and "OK" or ("*** want [" .. want .. "]/" .. wanthow)))
end
...
local g1, h1 = run(SRC); ck("first run (cold)", g1, h1, WANT, "cold")
local g2, h2 = run(SRC); ck("second run (warm)", g2, h2, WANT, "warm")

-- artifact really exists on disk
local f = io.open(path, "r"); local exists = f ~= nil; if f then f:close() end
...
local w = io.open(path, "w"); w:write("this is not valid lua ]["); w:close()
local g3, h3 = run(SRC); ck("corrupt artifact", g3, h3, WANT, "cold")
...
if Cache.artifact_path(SRC) == nil then print("  (no home -> nil path)") end
