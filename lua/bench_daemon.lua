...
do
  ...
  for _ = 1, 60 do local sh = rt.Shell.new(); sh.out = devnull; Cache.run(src, sh) end
end

local function now()
  local tv = ffi.new("struct td"); C.gettimeofday(tv, nil)
...
local t0 = now()
for _ = 1, N do
  local fds = ffi.new("int[2]"); C.pipe(fds)
  local pid = C.fork()
  if pid == 0 then                     -- worker: run the script, output -> pipe
    C.dup2(fds[1], 1); C.close(fds[0]); C.close(fds[1])
  ...
  C.close(fds[1])
  while C.read(fds[0], buf, 8192) > 0 do end   -- drain child stdout
  C.close(fds[0])
  C.waitpid(pid, st, 0)
end
local t1 = now()
