...
local lazy_mods = { "b_echo", "b_completion",
  ...
  "b_eval", "b_source", "b_wait", "b_fc", "b_bind", "b_shopt", "b_let", "b_kill",
  "b_pushd", "b_builtin", "b_pwd", "b_shift", "b_local", "b_help" }
...
if cdest then
  ...
  for i = 1, #bc do
    ...
    if #line == 24 then parts2[#parts2 + 1] = table.concat(line, ",") .. ",\n"; line = {} end
  end
  if #line > 0 then parts2[#parts2 + 1] = table.concat(line, ",") .. ",\n" end
  parts2[#parts2 + 1] = "};\n"
  local c = assert(io.open(cdest, "w")); c:write(table.concat(parts2)); c:close()
