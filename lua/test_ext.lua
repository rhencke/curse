...
local function I(src)
	local sh = T.rt.Shell.new()
	local b = {}
	sh.out = function(s)
		b[#b + 1] = s
	end
	T.interp.run(sh, T.parser.parse(src))
	return (table.concat(b):gsub("\n", "|"):gsub("|$", ""))
end
local function C(src)
	local sh = T.rt.Shell.new()
	local b = {}
	sh.out = function(s)
		b[#b + 1] = s
...
local ok = true
local function ck(d, src, e)
	local i, c = I(src), C(src)
	local p = (i == e and c == e)
	ok = ok and p
	print(("  %-24s interp=%-14s compiled=%-14s %s"):format(d, i, c, p and "OK" or ("*** want [" .. e .. "]")))
end
...
ck("$() builtin", 'x=$(echo hi)\necho "[$x]"', "[hi]")
ck("$() external", "n=$(expr 2 + 3)\necho $((n+2))", "7")
ck("$() inline", 'echo "a $(echo b) c"', "a b c")
ck("$() multiword", "y=$(echo one two)\necho [$y]", "[one two]")
if not ok then
	os.exit(1)
end
