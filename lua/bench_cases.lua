...
local function parse_cases(text)
	...
	for line in (text .. "\n"):gmatch("(.-)\n") do
		...
		if name then
			if cur then
				cases[#cases + 1] = { name = cur.name, code = table.concat(cur.code, "\n") }
			end
			cur = { name = name, code = {} }
			inMeta = false
		elseif cur then
...
local curse = function(mode)
	return LUAJIT .. " " .. RUNLUA .. " " .. codep .. " " .. mode
end
...
for _, path in ipairs(files) do
	for _, c in ipairs(parse_cases(readfile(path) or "")) do
		...
		if bout == iout and bst == ist then -- a passing case
			...
			rows[#rows + 1] =
				{ name = (path:match("[^/]+$") .. ": " .. c.name), bash = tb, interp = ti, tiered = tt, compiled = tc }
		end
	end
end
...
table.sort(rows, function(a, b)
	return (a.bash / a.compiled) > (b.bash / b.compiled)
end)
io.write("\nTop cases by bash/compiled speedup (compute-heavy):\n")
for i = 1, math.min(8, #rows) do
	local r = rows[i]
	io.write(
		("  %5.1fx  bash=%sms interp=%sms tiered=%sms compiled=%sms  %s\n"):format(
			r.bash / r.compiled,
			ms(r.bash),
			ms(r.interp),
			ms(r.tiered),
